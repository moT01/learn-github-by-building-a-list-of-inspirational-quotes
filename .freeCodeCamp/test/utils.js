...
const pathToRepo = '../inspirational-quotes';
sgf.cwd = pathToRepo;
...
const getCommandOutput = async function(command) {
  const { stdout } = await execute(command, { cwd: pathToRepo, shell: '/bin/bash' });
  return stdout;
}

const getGithubUsername = async function() {
  const { stdout } = await execute('git config user.name', { cwd: '../', shell: '/bin/bash' });
